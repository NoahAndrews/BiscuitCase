...
public enum ItemsDataSource {
    ...
    public void addCategory(Category category){
        ...
        Cursor c = database.rawQuery("SELECT * FROM " + CATEGORIES_TABLE + " ORDER BY " + COLUMN_CATEGORY_ID + " DESC LIMIT 1;",null);  //The last_insert_rowid SQL function won't work because Category IDs start from zero.
        c.moveToFirst();
        int id = c.getInt(0);
        category.setId(id);
